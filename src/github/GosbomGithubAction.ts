...
import { GosbomOptions } from "../Gosbom";
import { VERSION } from "../GosbomVersion";
...
import { downloadGosbomFromZip } from "./GosbomDownloader";
import { stringify } from "./Util";

export const GOSBOM_BINARY_NAME = "gosbom";
export const GOSBOM_VERSION = core.getInput("gosbom-version") || VERSION;

const PRIOR_ARTIFACT_ENV_VAR = "NEXTLINUX_SBOM_ACTION_PRIOR_ARTIFACT";
...
export function getArtifactName(): string {
  ...
  // __self __self_2 __nextlinux_sbom-action  __nextlinux_sbom-action_2 etc.
...
/**
 * Gets a reference to the gosbom command and executes the gosbom action
 * @param input gosbom input parameters
 * @param format gosbom output format
 * @param opts additional options
 */
async function executeGosbom({
  input,
  format,
  ...opts
}: GosbomOptions): Promise<string> {
  let stdout = "";
  ...
  if (registryUser) {
    env.GOSBOM_REGISTRY_AUTH_USERNAME = registryUser;
    if (registryPass) {
      env.GOSBOM_REGISTRY_AUTH_PASSWORD = registryPass;
    } else {
      core.warning(
  ...
  // https://github.com/nextlinux/gosbom#configuration
  ...
  // Execute in a group so the gosbom output is collapsed in the GitHub log
  ...
  const exitCode = await core.group("Executing Gosbom...", async () =>
    execute(cmd, args, {
      env,
      outStream,
      listeners: {
  ...
  if (exitCode > 0) {
    debugLog("Gosbom stdout:", stdout);
    throw new Error("An error occurred running Gosbom");
  } else {
    return stdout;
  }
}
...
/**
 * Downloads the appropriate Gosbom binary for the platform
 */
export async function downloadGosbom(): Promise<string> {
  const name = GOSBOM_BINARY_NAME;
  const version = GOSBOM_VERSION;

  const url = `https://raw.githubusercontent.com/nextlinux/${name}/main/install.sh`;
  ...
  const gosbomBinaryPath = `${installPath}_${name}`;

  await execute("sh", [installPath, "-d", "-b", gosbomBinaryPath, version]);

  return `${gosbomBinaryPath}/${name}`;
}

/**
 * Gets the Gosbom command to run via exec
 */
export async function getGosbomCommand(): Promise<string> {
  const name = GOSBOM_BINARY_NAME;
  const version = GOSBOM_VERSION;

  const sourceGosbom = await downloadGosbomFromZip(version);
  if (sourceGosbom) {
    core.info(`Using sourceGosbom: '${sourceGosbom}'`);
    return sourceGosbom;
  }

  let gosbomPath = cache.find(name, version);
  if (!gosbomPath) {
    // Not found; download and install it; returns a path to the binary
    gosbomPath = await downloadGosbom();

    // Cache the downloaded file
    gosbomPath = await cache.cacheFile(gosbomPath, name, name, version);
  }

  core.debug(`Got Gosbom path: ${gosbomPath} binary at: ${gosbomPath}/${name}`);

  // Add tool to path for this and future actions to use
  core.addPath(gosbomPath);
  return `${gosbomPath}/${name}`;
}

/**
 * Returns the SBOM format as specified by the user, defaults to SPDX
 */
export function getSbomFormat(): GosbomOptions["format"] {
  return (core.getInput("format") as GosbomOptions["format"]) || "spdx-json";
}
...
export async function runGosbomAction(): Promise<void> {
  ...
  const output = await executeGosbom({
    input: {
  ...
  if (output) {
    ...
    throw new Error(`No Gosbom output`);
  }
}
