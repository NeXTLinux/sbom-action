...
import * as action from "../src/github/GosbomGithubAction";
import {
  downloadGosbom,
  runAndFailBuildOnException
} from "../src/github/GosbomGithubAction";
...
describe("Action", () => {
  ...
  it("downloads gosbom", async () => {
    const path = await downloadGosbom();
    expect(path).toBe("download-tool_gosbom/gosbom")
  });
  ...
  it("runs with default inputs on push", async () => {
    setData({
      ...
      context: {
        ...
        action: "__nextlinux_sbom-action",
      },
    });

    await action.runGosbomAction();
  ...
  it("does not include docker scheme by default", async () => {
    ...
    expect(cmd.endsWith("gosbom")).toBeTruthy();
    expect(args).toContain("somewhere/org/img");
    expect(env.GOSBOM_REGISTRY_AUTH_USERNAME).toBeFalsy();
    expect(env.GOSBOM_REGISTRY_AUTH_PASSWORD).toBeFalsy();
  });
  ...
  it("uses registry scheme with username and password", async () => {
    ...
    expect(env.GOSBOM_REGISTRY_AUTH_USERNAME).toBe("mr_awesome");
    expect(env.GOSBOM_REGISTRY_AUTH_PASSWORD).toBe("super_secret");
  });
